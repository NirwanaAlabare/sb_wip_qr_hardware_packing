...
@section('custom-script')
    <script>
        ...
        Livewire.on('showSelectRejectArea', function (rejectAreaImage, x, y) {

            showSelectRejectArea(rejectAreaImage);

            setTimeout(() => {

                $('#reject-area-position-x').val(x);
                $('#reject-area-position-y').val(y);

                // tampilkan marker jika elemennya sudah ada
                let img = document.getElementById('reject-area-img');
                let point = document.getElementById('reject-area-img-point');

                if (img && point && x && y) {
                    let rect = img.getBoundingClientRect();

                    point.style.width = (0.03 * rect.width) + 'px';
                    point.style.height = point.style.width;
                    point.style.left = `calc(${x}% - ${0.015 * rect.width}px)`;
                    point.style.top = `calc(${y}% - ${0.015 * rect.width}px)`;
